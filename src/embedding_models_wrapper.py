...
class TransformersTextEmbedder(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        if "embeddinggemma-300m" in model.lower():
            dense_2_weight = load_file(os.path.join(model, "2_Dense/model.safetensors"))['linear.weight']
            dense_3_weight = load_file(os.path.join(model, "3_Dense/model.safetensors"))['linear.weight']
            self.dense_2_layer = torch.nn.Linear(dense_2_weight.size(1), dense_2_weight.size(0), bias=False).to(self.base_model.device)
            self.dense_2_layer.weight.data = dense_2_weight.to(self.base_model.device)
            self.dense_3_layer = torch.nn.Linear(dense_3_weight.size(1), dense_3_weight.size(0), bias=False).to(self.base_model.device)
            self.dense_3_layer.weight.data = dense_3_weight.to(self.base_model.device)

        self.tokenizer = AutoTokenizer.from_pretrained(model, **kwargs)

        if "bge-m3" in model.lower() or "bge-large-en-v1.5" in model.lower():
    ...
    def forward(
        ...
        **kwargs
        ...
        if "nv-embed-v2" in self.model_path.lower():
            embeddings = self.pooling(output['sentence_embeddings'], attention_mask)
        else:
            embeddings = self.pooling(output.last_hidden_state, attention_mask)
        if "embeddinggemma-300m" in self.model_path.lower():
            embeddings = self.dense_3_layer(self.dense_2_layer(embeddings))
...
class STWrapper(SentenceTransformerEncoderWrapper):
        ...
        **kwargs,  # For `TransformersTextEmbedder`
        ...
        if not use_instruction:
            self.mteb_model_meta = ModelMeta(
                name=f"local/{model_name}", revision="wo_inst", release_date=None, languages=None, n_parameters=None, memory_usage_mb=None, max_tokens=None, embed_dim=None, license=None, open_weights=False, public_training_code=None, public_training_data=None, framework=["Sentence Transformers"], similarity_fn_name="cosine", use_instructions=True, training_datasets=None, loader=None
            )
        else:
            self.mteb_model_meta = ModelMeta(
                name=f"local/{model_name}", revision="w_inst", release_date=None, languages=None, n_parameters=None, memory_usage_mb=None, max_tokens=None, embed_dim=None, license=None, open_weights=False, public_training_code=None, public_training_data=None, framework=["Sentence Transformers"], similarity_fn_name="cosine", use_instructions=True, training_datasets=None, loader=None
            )
    ...
    def get_instruction(self, task_name, prompt_type, hf_subset):
        if prompt_type != 'query':
            return ""
        if task_name=="EPBench" or task_name=="KnowMeBench" or task_name=="ReMe":
            hf_subset = hf_subset.split("/")[-1]
        if task_name=="MemGovern":
            hf_subset = "MemGovern"
        instruction = self.instruction_dict[task_name][hf_subset]
        return instruction
    ...
    def encode(
        ...
        **kwargs: Any,
        ...
        if self.use_instruction:
            instruction = self.get_instruction(task_name, prompt_type, hf_subset)
            if self.instruction_template:
                instruction = self.format_instruction(instruction, prompt_type)
            logger.info(f"Using instruction: '{instruction}' for task: '{task_name}, {hf_subset}'")
